...
class ProductModel {
  ...
  async list_products(limit: number = 12, page: number = 1) {
    const q = `SELECT t0.id,t0.name,t0.price,t0.quantity,t0.status,t0.brand,t0.thumbnail,t0.slug,((1-COALESCE (t2.percent_off,0))-COALESCE (t2.amount_off,0)) AS sale_off
              FROM products as t0
              LEFT JOIN(
  ...
  async product_detail(id: any) {
    const q = `SELECT t0.id, t0.name,t0.quantity,t0.price,((1-COALESCE (t2.percent_off,0))-COALESCE (t2.amount_off,0)) AS sale_off, t0.description,t0.status,array_cat(t0.thumbnail,t0.images) as images,t1.comments, t1.avg_rate
                FROM products AS t0
                LEFT JOIN (
